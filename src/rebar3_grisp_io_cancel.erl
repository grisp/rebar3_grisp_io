...
init(State) ->
    Provider = providers:create([
        ...
        {module, ?MODULE},
        {bare, true},
        {example, "rebar3 grisp-io cancel -d DEVICE"},
        {opts, options()},
        {profile, [default]},
        {short_desc, "Cancel a device update"},
        {desc, "Cancel the update running on a device linked to grisp.io"}
    ]),
...
do(RState) ->
    ...
    try
        ...
        throw:forbidden ->
            abort("Error: No permission to cancel this update");
        throw:device_does_not_exist ->
            abort("Error: The given device does not exist or is not linked");
        throw:{error, Reason} ->
            abort("Error: cancellation request rejected: ~s", [Reason])
    end.
...
options() -> [
    {device, $d, "device", string,
     "Identifier of the device whose update should be cancelled"}
].
